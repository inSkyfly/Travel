...
class LlmClient:
    ...
    def chat_stream(
        ...
        print("have client?", self._client is not None)
        if self._client is None:
    ...
    def _mock_generate_plan(
        ...
        names = [a["name"] for a in attractions_db] if attractions_db else []
        if len(names) < 2:
            names = self._extract_place_names(rag_summary, destination)
        if len(names) < 2:
            names = self._default_places_for(destination)

        food_names = [f["name"] for f in foods_db] if foods_db else []
        if not food_names:
            food_names = self._extract_food_hints(rag_summary, destination)
        ...
        for i in range(day_count):
            ...
            evening_food = food_names[i % len(food_names)] if food_names else f"{destination}本地特色餐厅"
            daily_plans.append(
                {
                    "dayIndex": i + 1,
                    "date": d.isoformat(),
                    "activities": [
                        ...
                        },
                        {
                            ...
                            "description": "",
                            "transportToNext": "",
                            "nextDestinationName": evening_food,
                            "nextDestinationLat": 0.0,
                            "nextDestinationLng": 0.0,
                            "transportMode": "NONE",
                        },
        ...
        attractions = (
            [dict(a) for a in attractions_db[:8]]
            if attractions_db
            else [
                {
                    "name": name,
        ...
        return {
            ...
            "foods": (
                [dict(f) for f in foods_db[:6]]
                if foods_db
                else [
                    {
                        "name": food_names[0] if food_names else f"{destination}特色餐厅",
                        "area": "市中心",
                        "taste": "当地风味",
                        "mealType": "正餐",
                        "avgPrice": 60,
            ...
            "budgetBreakdown": {
                ...
                "categories": [
                    {"name": "交通", "allocated": 1800, "spent": transport_price * 2},
                    {
                        "name": "住宿",
                        "allocated": 1800,
                        "spent": 420 * max(day_count - 1, 1),
                    },
                    {
                        "name": "餐饮",
                        "allocated": 1200,
                        "spent": 80 * day_count * travelers,
                    },
    ...
    def _extract_food_hints(text: str, destination: str) -> list[str]:
        ...
        for line in text.splitlines():
            if any(
                k in line for k in ("美食", "小吃", "餐厅", "火锅", "面", "烤鸭", "菜")
            ):
    ...
    def _default_places_for(destination: str) -> list[str]:
        presets: dict[str, list[str]] = {
            ...
            "新疆": ["天山天池", "喀纳斯湖", "赛里木湖", "那拉提草原", "国际大巴扎"],
            "伊犁": ["那拉提草原", "赛里木湖", "喀纳斯湖", "天山天池"],
            "乌鲁木齐": ["新疆国际大巴扎", "天山天池", "红山公园"],
        }
